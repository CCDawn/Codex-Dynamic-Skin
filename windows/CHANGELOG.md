...
- 远程桌面（RDP）会话自动降级为静态帧：RDP 会话经 WARP 软件合成渲染，全窗口视频壁纸会持续消耗 CPU 并淹没远程画面流。现在检测到远程会话时，视频主题改为解码单帧海报（`media.transport="poster"`），捕帧后回收视频元素与 Blob，前景半透明效果保持不变；回到本地控制台会话后托盘自动恢复视频播放。托盘每分钟检查会话状态，切换时重新应用并气泡提示。

## 1.6.1 — 2026-09-21

### 修复

- 视频壁纸现在正确透进前景内容区：视频主题此前不参与图片宽幅分析，`data-dream-art-wide` 保持 `false`，内容面板保留不透明底色，导致视频只在窗口四周可见；现在视频主题一律按宽幅艺术处理，主表面与侧栏使用沉浸式半透明渐变。
- 恢复合并时丢失的 `OPERATION_UI_REGISTRY_KEY` 常量：此前托盘/管理器执行应用、暂停、切换主题时的页面内进度指示（operation UI）表达式引用未定义常量而静默失败，状态浮层不再出现。

## 1.6.0 — 2026-09-21

### 新增

- 合并上游 Codex Dream Skin v1.5.18：选择器合同引擎（`selectors.json` 三重兜底，覆盖 2026 年 9 月版 Codex 26.915 的 DOM 变化）、受管 CDP 配置、社区主题 ZIP 导入与 Safe CSS 校验、中英文多语言托盘，以及更丰富的主题字段（副标题/口号/主题色）。
- 产品品牌统一为「Codex 动态壁纸」：托盘、安装向导、开始菜单与开机自启快捷方式、更新提示、默认主题名全部使用本仓库品牌；更新检查改指 `CCDawn/Codex-Dynamic-Skin` releases，不再误报上游版本。
- 安装器支持从上游 v1.5.18 Inno 安装原地升级（版本号 1.6.0 > 1.5.18），升级与卸载时会自动清理旧的英文命名快捷方式。

### 修复

- Wallpaper Engine 创意工坊目录缺失或引用失效时，自动回退到主题目录中的托管媒体副本，不再因工坊源失效而启动失败；目录边界与 reparse point 安全检查保持不变。
