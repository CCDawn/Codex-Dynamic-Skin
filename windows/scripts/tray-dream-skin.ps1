...
try {
  ...
  function Set-DreamSkinAutoStart {
    ...
    $shortcut.Description = '在通知区域启动 Codex 动态壁纸'
    $shortcut.Save()
  }
  ...
  function Add-DreamSkinRevealControl {
    ...
    $label = [System.Windows.Forms.ToolStripLabel]::new((Get-DreamSkinTrayText -Key 'RevealLabel' -FormatArguments @($percent)))
    ...
    $trackBar.AccessibleName = 'Wallpaper reveal'

    $updateLabel = {
      $label.Text = (Get-DreamSkinTrayText -Key 'RevealLabel' -FormatArguments @($trackBar.Value))
    }.GetNewClosure()
    $commitReveal = {
      try {
        $label.Text = (Get-DreamSkinTrayText -Key 'RevealLabel' -FormatArguments @($trackBar.Value))
  ...
  function Rebuild-DreamSkinTrayMenu {
    ...
    $null = Add-DreamSkinTrayItem -Items $menu.Items -Text (Get-DreamSkinTrayText -Key 'Apply') -Action {
      ...
      $notify.ShowBalloonTip(1800, 'Codex 动态壁纸', (Get-DreamSkinTrayText -Key 'Applying'), [System.Windows.Forms.ToolTipIcon]::Info)
    }
    ...
    if ($paused) {
      ...
      $null = Add-DreamSkinTrayItem -Items $menu.Items -Text (Get-DreamSkinTrayText -Key 'Pause') -Action {
        ...
        $notify.ShowBalloonTip(2800, 'Codex 动态壁纸', $removalMessage, $icon)
    ...
    $null = Add-DreamSkinTrayItem -Items $menu.Items -Text (Get-DreamSkinTrayText -Key 'ChangeBackground') -Action {
      ...
      try {
        if ($dialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
          ...
          $notify.ShowBalloonTip(1800, 'Codex 动态壁纸', (Get-DreamSkinTrayText -Key 'BackgroundUpdated'), [System.Windows.Forms.ToolTipIcon]::Info)
        }
      } finally {
        $dialog.Dispose()
      }
    ...
    $null = Add-DreamSkinTrayItem -Items $menu.Items -Text (Get-DreamSkinTrayText -Key 'ImportZip') -Action {
      ...
      $dialog.Filter = 'Codex 动态壁纸主题 ZIP|*.zip'
      $dialog.Multiselect = $false
      try {
        if ($dialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
          ...
          $notify.ShowBalloonTip(4200, 'Codex 动态壁纸', $message, $messageIcon)
        }
      } finally {
        $dialog.Dispose()
      }
