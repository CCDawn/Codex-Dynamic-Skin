...
function Get-DreamSkinWallpaperEngineReference {
  ...
  $fullWorkshopRoot = [System.IO.Path]::GetFullPath($workshopRoot).TrimEnd('\')
  $workshopDirectory = [System.IO.Path]::GetFullPath((Join-Path $fullWorkshopRoot $workshopId))
  # Containment is pure path math; existence is a separate concern so a missing
  # Workshop item can fall back to the managed theme copy instead of throwing.
  if (-not $workshopDirectory.StartsWith($fullWorkshopRoot + '\', [System.StringComparison]::OrdinalIgnoreCase)) {
    throw 'Wallpaper Engine Workshop item escaped its content directory.'
  }
  $mediaPath = [System.IO.Path]::GetFullPath((Join-Path $workshopDirectory $relativePath))
  if (-not $mediaPath.StartsWith($workshopDirectory.TrimEnd('\') + '\', [System.StringComparison]::OrdinalIgnoreCase)) {
    throw 'Wallpaper Engine media escaped its item directory.'
  }
  if (-not (Test-Path -LiteralPath $mediaPath -PathType Leaf)) {
    # The Steam Workshop item disappeared (Steam uninstalled or unsubscribed);
    # treat the theme as a plain managed media theme from now on.
    return $null
  }
...
function Set-DreamSkinActiveTheme {
  ...
  try {
    ...
    # Keep the lean community contract intact for themes that never carried a
    # media object; dynamic (video) themes and existing media contracts keep one.
    if ($null -ne $Theme.media -or $mediaType -ceq 'video') {
      $Theme | Add-Member -NotePropertyName media -NotePropertyValue `
        ([pscustomobject]@{ type = $mediaType; playbackRate = if ($Theme.media -and $Theme.media.playbackRate) {
